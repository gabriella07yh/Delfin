...
class BuscarController {
    ...
    //imprime: pagina de resultados con filtros aplicados y paginacion
    public function index(): void {
        //consultar: leer parametros GET para construir los filtros
        $filtros = [
            'q'              => trim($_GET['q']              ?? ''),
        ...
        //consultar: obtener resultados y total desde el modelo con filtros
        $revistas    = Revista::obtenerTodas($filtros, $pagina);
        $total       = Revista::contar($filtros);
        $totalPaginas= (int)ceil($total / ITEMS_POR_PAGINA);
        $areas       = Revista::obtenerAreas();
        $paises      = Revista::obtenerPaises();
